...
giustezza <- risultati[tolower(parametro) %like% "giustezza",
                        # calcolo le metriche prestazionali
                        .(
                         parametro = "Giustezza",
...
prestazioni <- rbind(normalita, precisione, giustezza, loq, intervalli) |>
...
lista_grafici <- risultati[parametro %like% "Precisione", 
  .(grafico_obj = list(plot_dispersione(
      .SD,
      livello = "livello",
...
for (i_matrice in matrici) {
  ...
  grafici_matrice <- lista_grafici[matrice == i_matrice]
  ...
  for (i_analita in analiti_matrice) {
    ...
    env_child$grafici <- grafici_matrice[analita == i_analita]
